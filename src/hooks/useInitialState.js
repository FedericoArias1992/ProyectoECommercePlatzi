...
const initialState = {
  //creamos el objeto que populamos con la info necesaria (el carrito)
  cart: [], //lo inicializamos vacio
  //info_persona:[], por ejemplo aca pueden venir más datos que caractericen el estado
};

const useInitialState = () => {
  //creamos el estado y la inicializamos como el estado initialState con cart vacio
  const [state, setState] = useState(initialState);

  const addToCart = (payload) => {
    //creamos la funcion que nos permitirá agregar items al carrito, recibe un objeto "product" - la variable por defecto se llama payload, pero es producto en este caso
    setState({
      ...state, //denota que mantenga lo que está en el estado (como guardado previamente)
      cart: [...state.cart, { ...payload, idCart: state.cart.length + 1 }], //que mantenga lo que habia en cart y agregue el product y
